...
def counter2d_in_sparse_mat(int_arr_2d, shape=None):
    ...
    count_mat = coo_matrix((np.ones(len(int_arr_2d)), (int_arr_2d.T[0], int_arr_2d.T[1])), shape=shape)
    return count_mat
...
def index_mats_or(mats_in):
    ...
    res_mat = coo_matrix((np.ones(len(index_tuples)), (index_tuples["row"], index_tuples["col"])),
                         shape=mats_in[0].shape)
    return res_mat
